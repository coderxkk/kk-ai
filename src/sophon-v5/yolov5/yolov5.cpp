...
int YoloV5::prepare_data(const std::vector<bm_image>& images){
  ...
  for(int i = 0; i < image_n; ++i) {
    ...
    bmcv_rect_t crop_rect{0, 0, (unsigned int)image1.width, (unsigned int)image1.height};
...
void YoloV5::draw_bmcv(bm_handle_t &handle, int classId, float conf, int left, int top, int width, int height, bm_image& frame, float draw_thresh, bool put_text_flag)   // Draw the predicted bounding box
{
  ...
  if (put_text_flag){
    ...
    bmcv_point_t org = {(int)rect.start_x, (int)rect.start_y};
    bmcv_color_t color = {(unsigned char)colors[classId % colors_num][0], (unsigned char)colors[classId % colors_num][1], (unsigned char)colors[classId % colors_num][2]};
    float fontScale = 2; 
